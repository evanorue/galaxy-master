...
public class GalaxyModel {
    ...
    public enum Forecast { DROUGHT, RAINY, OPTIMAL, NORMAL, UNKNOWN }
    ...
    private double getArea(int days) {
        Point2D F = FERENGI.getPosition(days);
        Point2D V = VULCANO.getPosition(days);
        Point2D B = BETASOIDE.getPosition(days);
    ...
    private Alignment getAlignment(int days) {
        ...
        double w1 = FERENGI.angularVelocity;
        double w2 = VULCANO.angularVelocity;
        double w3 = BETASOIDE.angularVelocity;
        ...
        if (aligned) {
            ...
            double sunSlope = getSlope(FERENGI.getPosition(t), SUN);
    ...
    private double getSlope(Planet pa, Planet pb, double days) {
        Point2D a = pa.getPosition(days);
        Point2D b = pb.getPosition(days);
